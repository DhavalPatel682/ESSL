<template>
  <div class="media-container">
    <h1 class="title">Event Media</h1>
    <div v-if="eventMedia.length === 0" class="no-sponsors">
      <p>No Event-Media available.</p>
    </div>
    <div class="sponsors-list">
      <transition-group name="fade" tag="div" class="media-content">
        <div class="media-card" v-for="(event, index) in eventMedia">
          <img :src="event.file_url" :alt="'Event ' + event.id" class="media-image" />
        </div>
      </transition-group>
  ...
  setup() {
    ...
    const fetchEventMedia = async () => {
      try {
        ...
        eventMedia.value = mediaData.filter(item => item.type === 'image');
      }
      catch (error) {
